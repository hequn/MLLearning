...
cv2.inRange(hsv, np.array([150, 140, 140], np.uint8), np.array([180, 254, 254], np.uint8), threshold_img) # xiong di mu ye
...
mask = np.zeros(trans.shape, dtype=np.uint8)
...
for idx in range(len(cnts)):
    ...
    mask[y:y + h, x:x + w] = 0
    # cv2.imshow("Image5", mask)
    # cv2.waitKey(1000)
    cv2.drawContours(mask, cnts, idx, (255, 255, 255), -1)
    r = float(cv2.countNonZero(mask[y:y + h, x:x + w])) / (w * h)
    # cv2.imshow("Image5", mask)
    # cv2.waitKey(1000)
    if r > 0.5 and w > 50 and h < 50 and h > 20 and y > 280 and y < 500:  # change the w h x y for the upper yellow digits
...
for (i, (gX, gY, gW, gH)) in enumerate(locs):
    ...
    cv2.waitKey(1000)
    print(pytesseract.image_to_string(group, lang='eng',
